...
class QLearning(object):
    def __init__(self, state_dim, action_dim, cfg, epsilon_start=0.9,
                 epsilon_end=0.1,
                 epsilon_decay=200):
    ...
    def update(self, state, action, reward, next_state, done):
        ############################ Q表格的更新方法（需要完成）##################################
        utils.CliffWalkingWapper(self.Q_table)
        Q_predict = self.Q_table[state, action]
        if done:
            Q_target = reward  # 没有下一个状态了
        else:
            Q_target = reward + self.gamma * np.max(self.Q_table[next_state, :])  # Q-table-learning
