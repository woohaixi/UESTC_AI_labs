...
# ID3算法构建决策树，加入剪枝和深度限制
def id3(data, attributes, label_col, max_depth=None, min_samples_split=2, current_depth=0):
    labels = data[label_col]
    # 如果所有样本的标签相同，返回叶节点
    ...
    if pd.api.types.is_numeric_dtype(data[best_attr]):
        ...
        if len(left_data) > 0:
            node.branches['<='] = id3(left_data, attributes - {best_attr}, label_col, max_depth, min_samples_split,
                                      current_depth + 1)
        if len(right_data) > 0:
            node.branches['>'] = id3(right_data, attributes - {best_attr}, label_col, max_depth, min_samples_split,
                                     current_depth + 1)
    else:
        # 处理离散属性
        ...
        for val in data[best_attr].unique():
            ...
            if len(subset) == 0:
                ...
                node.branches[val] = id3(subset, attributes - {best_attr}, label_col, max_depth, min_samples_split,
                                         current_depth + 1)
...
def main():
    ...
    tree = id3(train_data, attributes, label_col, max_depth=5, min_samples_split=2)
