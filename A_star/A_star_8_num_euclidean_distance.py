...
def get_euclidean_distance(state, goal_state, n):
    """
    计算当前状态下所有数码块到目标位置的欧几里得距离之和（忽略空格0）。
    """
    distance = 0
    for i in range(n):
        for j in range(n):
...
def solve_n_puzzle(initial_state, goal_state):
    ...
    closed_list = {}
    g_start = 0
    h_start = get_euclidean_distance(initial_state, goal_state, n)
    ...
    while open_list:
        ...
        for neighbor_state in neighbors:
            ...
            h_neighbor = get_euclidean_distance(neighbor_state, goal_state, n)
